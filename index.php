...
.nav-left {
    position: relative;
    text-shadow: 
        0 0 5px var(--emas-soft),
        0 0 12px var(--emas-bright),
        0 0 18px var(--emas-glow);
    animation: sipusGlow 3s ease-in-out infinite alternate;
}
@keyframes sipusGlow{
    0%{ text-shadow:0 0 6px var(--emas-soft); }
    100%{ text-shadow:0 0 18px var(--emas-bright); }
}
...
.navbar a:hover{color:var(--emas-bright);}
.btn-control{
    padding:8px 14px;
...
.section-title{
    ...
    font-weight:bold;
    color:var(--emas-bright);
    margin-top:100px; /* agak kebawah supaya hero jadi fokus */
}
.book-container{
    margin-top:30px;
    display:flex;
...
/* ===== ANIMASI FADE UP ===== */
const fadeElems = document.querySelectorAll(".book-card, .section-title, .hero h1, .hero .slogan, .hero .btn");
...
window.addEventListener("scroll", ()=>{
    ...
    document.querySelector(".hero").style.transform = `translateY(${scrollVal * 0.25}px)`;
});
